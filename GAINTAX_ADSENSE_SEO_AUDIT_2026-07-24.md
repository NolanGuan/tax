...
- **Production deployment and live verification: pass.** Vercel deployed commit `dab07e1`. All 21 sitemap URLs return direct `200` with one H1, matching canonical, indexable metadata, and parseable JSON-LD; 22 unique internal targets have no broken link and no sitemap page is orphaned. The corrected query URL returns `200`, all intended legacy paths return `308`, the invalid route returns `404`, and both image assets return `image/png`. Browser, Googlebot, and Bingbot receive the same blog identity. Desktop and 390 px browser checks, the editorial link, and a representative calculator result passed with zero console warnings/errors.
- **English-only project verification: pass.** Repository text and all 21 production sitemap pages contain no Han characters. Every production page declares `lang="en"`; rendered mobile UI, privacy controls, navigation, and calculator results are English-only. Preflight now rejects Han characters in project text files and reports each file and line.
